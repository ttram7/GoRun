...
router.get('/', (req, res) => {
  ...
  if (req.isAuthenticated()) {
    ...
    const query = `select * from "runs" where "date" > now() - interval '1 week'AND "user_id" = $1 ORDER BY "date" DESC`;
...
//   const sqlText = `SELECT "date", "distance", ('{SU,M,T,W,TH,F,SA}'::text[])[extract(dow from "date") + 1] as dow_name, 
//   EXTRACT (DOW FROM "date") AS dow_num FROM "runs" WHERE EXTRACT (WEEK FROM "date") = EXTRACT (WEEK FROM NOW() - INTERVAL '1 week') AND "user_id" = $1 ORDER BY "date" ASC;`
//   pool.query(sqlText, [req.user.id])
//       .then((result) => {
//           //console.log(`Got stuff back from the database`, result.rows);
//           res.send(result.rows);
