...
router.get('/', (req, res) => {
    ...
    //const sqlText = `SELECT * FROM "runs" WHERE EXTRACT (WEEK FROM "date") = EXTRACT (WEEK FROM NOW()) AND "user_id" = $1 ORDER BY date ASC;`
    // const sqlText = `SELECT "date", "distance", EXTRACT (DOW FROM "date") AS dow FROM "runs" 
    // WHERE EXTRACT (WEEK FROM "date") = EXTRACT (WEEK FROM NOW()) AND "user_id"=$1 ORDER BY "date" ASC;`
